...
import { stableKeyFromParts, stableKeyFromText } from "@/lib/keys"
								...
								<div className={cn("flex flex-col", ui.gap.sm)}>
									...
									>
										...
										<PbTabPanel size="sm" className="min-w-0 flex flex-col">
											...
											<p role="status" aria-live="polite" className={cn(ui.margin.topXs, "leading-snug text-muted-foreground", ui.typography.caption)}>
												{viewer_status_copy}
											</p>
											...
											<div className="mt-3 flex min-h-0 min-w-0 flex-1 flex-col" style={viewer_pane_style}>
												<div className={cn("min-h-0 flex-1 overflow-hidden bg-background flex flex-col", ui.radius.base, ui.surface.structure.shadowNone, ui.surface.structure.border)}>
													<div aria-busy={is_running ? "true" : undefined} className="relative flex min-h-0 flex-1 flex-col">
														{is_running && (
															<div aria-hidden="true" className="absolute inset-0 z-10 flex flex-col gap-px overflow-hidden bg-background">
																{Array.from({ length: 12 }, (_, i) => (
																	<div
																		key={i}
																		className="h-8 w-full animate-[shimmer_1.4s_linear_infinite] bg-[length:200%_100%] bg-[linear-gradient(90deg,transparent_25%,var(--muted)_50%,transparent_75%)]"
