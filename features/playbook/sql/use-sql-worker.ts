	...
	const ensure_ready = React.useCallback(async () => {
		...
		if (!init_promise_ref.current) {
			...
			init_promise_ref.current = new Promise<void>((resolve, reject) => {
				const on_message = (e: MessageEvent<SqlWorkerOutboundMessage>) => {
					const msg = e.data

					if (msg.type === "ready") {
						worker.removeEventListener("message", on_message as unknown as EventListener)
						resolve()
						return
					}
				...
				worker.addEventListener("message", on_message as unknown as EventListener)
